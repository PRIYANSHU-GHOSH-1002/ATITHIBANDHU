...
const { sendSOS } = require('../services/sms');
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const populatedAlert = await alert.populate('tourist', 'name phone nationality emergencyContact');

    const io = req.app.get('io');
    if (io) io.to('admins').emit('sos:alert', populatedAlert);

    // Send SMS to the tourist's registered emergency contact
    const tourist = populatedAlert.tourist;
    if (tourist?.emergencyContact?.phone) {
      const mapsLink = `https://www.google.com/maps?q=${lat},${lng}`;
      const smsBody = `🚨 SOS ALERT: ${tourist.name} has triggered an emergency alert via AtithiBandhu. Location: ${mapsLink}`;

      sendSOS(tourist.emergencyContact.phone, smsBody).catch((err) => {
        console.error('SOS SMS failed:', err.message);
      });
    } else {
      console.warn(`No emergencyContact on file for tourist ${tourist?._id}`);
    }
