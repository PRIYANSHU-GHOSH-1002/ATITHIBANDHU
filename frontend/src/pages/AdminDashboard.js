...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    socket.on('admin:aiSafetyUpdate', (data) => {
      if (data.status === 'safe') return;
      const scoreText = typeof data.riskScore === 'number' ? ` (score ${data.riskScore.toFixed(2)})` : '';
      setAnomalies((prev) => [
        { ...data, reason: `AI model flagged ${data.status.toUpperCase()}${scoreText}${data.message ? ` — ${data.message}` : ''}` },
        ...prev,
      ].slice(0, 20));
    });
    ...
    return () => {
      ...
      socket.off('admin:aiSafetyUpdate');
    };
  }, []);
  ...
  return (
    <div className="dashboard admin">
      <div className="top-bar">
        ...
        <span className="live-pill"><span className="pulse-dot" />{markers.length} tracked live</span>
      </div>
      ...
      <div className="admin-grid">
        <div className="admin-map">
          <MapView center={[20.5937, 78.9629]} zoom={5} zones={zones} markers={markers} height="520px" label="All tourists, live" />
        </div>

        <div className="admin-panel">
          <section>
            <h3>Active SOS alerts <span className="badge">{sosAlerts.length}</span></h3>
          ...
          <section>
            ...
            <ul className="feed">
              {anomalies.map((a, i) => (
                <li key={i}>#{String(a.touristId).slice(-6)} — {a.reason}</li>
              ))}
            </ul>
          </section>
          ...
          <section>
            ...
            <form onSubmit={createZone} className="zone-form">
              ...
              <textarea
                placeholder="lat,lng; lat,lng; lat,lng (min 3 points)"
          ...
          <section>
            <h3>Registered tourists <span className="badge">{tourists.length}</span></h3>
            <ul className="feed">
              {tourists.map((t) => (
                <li key={t._id}>
                  {t.name} — {t.status}{liveLocations[t._id] ? ' · live' : ''}
                </li>
              ))}
            </ul>
          </section>
